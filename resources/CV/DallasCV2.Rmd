...
\begin{center}
\begin{tabular}{ccccc}
  Institution & Mentor & Location & Degree or Position & Completion Date   \\
  \hline
  Truman State University & -- & Kirksville, MO. & Biology. B.Sc. & 2009 \\
  Truman State University & Stephanie Fore & Kirksville, MO. & Biology. M.Sc. & 2010 \\
  University of Georgia & John Drake & Athens, GA & Ecology Ph.D & 2016\\
  University of California & Alan Hastings & Davis & Postdoctoral Researcher & 2018\\
  University of Helsinki & Otso Ovaskainen & Finland & Postdoctoral Researcher & 2019 \\
  \hline
\end{tabular}
\end{center}
...
#    x$title <- gsub("</i>", "}", x$title)
  ...
  x$journal <- sub('&', '\\&', x$journal, fixed=TRUE)
  
  doi <- try(x$labels[[2]]$doi)
  if(inherits(doi, 'try-error')){
    doi <- NULL
  }
  return(paste0(c("\\item ", 
        auth, '. \\textbf{', x$date,'}. ', x$title, '.  \\textit{',x$journal, '} . doi: ', doi), 
          collapse='')
        )
}
...
prez$Date <- format(paste(prez$Date, ', ', prez$Month, sep=''))
...
vitae::detailed_entries(data=teaching, 
  what = course, 
  when = year,
  with = students) |> 
  arrange(desc(when))
...
if(admin){
  funding$totalMoney <- paste('$', formatC(funding$Amount, format='d', big.mark=','), 
    ' ($', trimws(funding$AmountToMe), ')', sep='')
}else{
  funding$totalMoney <- paste('$', formatC(funding$Amount, format='d', big.mark=','), sep='')
}
...
vitae::detailed_entries(data=mentoring,  
  what = Student, 
  when = DateRange,
  with= Type)|> 
  arrange(desc(when))
...
if(any(ugmentoring$Start==ugmentoring$End)){
  inds <- which(ugmentoring$Start==ugmentoring$End)
  ugmentoring$DateRange[inds] <- paste(ugmentoring$Start[inds], sep='')
}

vitae::detailed_entries(data=ugmentoring,  
  what = Student, 
  when = DateRange,
  with= Major) |> 
  arrange(desc(when))
